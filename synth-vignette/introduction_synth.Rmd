...
ebdatsub <- read.csv(file=paste0(datapath, 'synth_test.csv'))
...
```{r, eval=F, echo=T}
## load the user IDs that we've saved from the sample:

ids <- read.csv(paste0(datapath, "eurob_sample_ids.csv"))

## load idvs:

load(paste0(datapath, "ebdatDemographVaribs.rda"))

## take the data subset:
idvs_subset <- idvs[which(idvs$uniqid %in% ids$uniqid),]

dim(idvs_subset) ## 1936 x 11

## clean up:
rm(idvs)
rm(ids)

thetas <- cbind(avgthetas, idvs_subset)
